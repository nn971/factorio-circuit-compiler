...
def test_event_causality_and_compilation_errors_are_distinct() -> None:
    ...
    circuit.output("constant", 0)
    module = circuit.build()
    with pytest.raises(EventCompilationError, match="requires exactly one Event update transition"):
...
def test_sample_on_direct_ir_validation_and_sample_only_boundary() -> None:
    ...
    assert compiled.abstract_physical.outputs == []
    timing = StateTimingPlan((), ())
    for route in (
        lambda: compile_legacy_circuit(module),
        lambda: optimize_semantic(module),
...
def test_sample_on_can_be_emitted_but_not_used_as_capture_trigger() -> None:
    capture_circuit = Circuit("sample_on_capture_boundary")
    capture_value = capture_circuit.input("value")
    capture_trigger = capture_circuit.event("trigger", guaranteed_min_separation=1)
    capture_crossing = capture_circuit.sample_on(capture_value, capture_trigger)
    memory = capture_circuit.freeze("memory")
    with pytest.raises(EventCrossingError, match="capture trigger"):
        memory.capture_on(capture_crossing, required_min_separation=1)  # type: ignore[arg-type]

    circuit = Circuit("sample_on_emit_boundary")
    value = circuit.input("value")
    trigger = circuit.event("trigger", guaranteed_min_separation=1)
    crossing = circuit.sample_on(value, trigger)
    circuit.output("crossing", crossing)
    module = circuit.build()
    assert module.output.values == (crossing.ir,)
    compiled = compile_circuit(module)
