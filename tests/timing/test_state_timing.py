...
def test_read_after_update_at_same_freshness_is_rejected() -> None:
    c = Circuit("too_early")
    data = c.signals("data")
    enable = c.input("enable")
    memory = c.freeze("memory")
    memory.set(data, when=enable)
    c.output("new", memory.value)

    with pytest.raises(StateTimingError, match="advance the freshness cursor"):
