"""Compare baseline and reach-safe proposal backoff on identical layout seeds."""
...
@contextmanager
def _backoff_disabled() -> Iterator[None]:
    """Temporarily restore the pre-experiment wire-reach rejection behavior."""

    original = incremental._reach_safe_backoff_target

    def disabled(*args: Any, **kwargs: Any) -> None:
        return None

    incremental._reach_safe_backoff_target = disabled
    try:
        yield
    finally:
        incremental._reach_safe_backoff_target = original


def _run(factory: CaseFactory, *, proposals: int, seed: int, backoff: bool):
    case = factory()
...
def _run_case(
    ...
    *,
    ...
    for seed in range(first_seed, first_seed + seeds):
        baseline, baseline_time = _run(factory, proposals=proposals, seed=seed, backoff=False)
        repaired, repaired_time = _run(factory, proposals=proposals, seed=seed, backoff=True)
        baseline_metrics = baseline.optimization.after
        repaired_metrics = repaired.optimization.after
        if repaired_metrics.objective < baseline_metrics.objective:
            outcome = "better"
        elif repaired_metrics.objective > baseline_metrics.objective:
            outcome = "worse"
        else:
            outcome = "equal"
        outcomes[outcome] += 1
        relay_deltas.append(repaired_metrics.relay_count - baseline_metrics.relay_count)
        area_deltas.append(repaired_metrics.occupied_area - baseline_metrics.occupied_area)
        wire_deltas.append(repaired_metrics.wire_length - baseline_metrics.wire_length)
        reach_rejection_deltas.append(
            repaired.stats.wire_reach_rejections - baseline.stats.wire_reach_rejections
        )
        queue_pop_deltas.append(
            repaired.stats.routing_queue_pops - baseline.stats.routing_queue_pops
        )
        runtime_ratios.append(repaired_time / baseline_time if baseline_time else 1.0)
        attempts += repaired.stats.reach_backoff_attempts
        feasible += repaired.stats.reach_backoff_feasible_targets
        accepted += repaired.stats.reach_backoff_moves_accepted
        print(
            f"{name} seed={seed}: baseline={baseline_metrics.objective}, "
            f"backoff={repaired_metrics.objective}, outcome={outcome}, "
            f"backoff={repaired.stats.reach_backoff_moves_accepted}/"
            f"{repaired.stats.reach_backoff_feasible_targets}/"
            f"{repaired.stats.reach_backoff_attempts}, "
            f"reach-reject={baseline.stats.wire_reach_rejections}->"
            f"{repaired.stats.wire_reach_rejections}, "
            f"queue-pops={baseline.stats.routing_queue_pops}->"
            f"{repaired.stats.routing_queue_pops}, "
            f"runtime={baseline_time:.3f}s->{repaired_time:.3f}s"
        )
    ...
    print(
        ...
        f"backoff={accepted}/{feasible}/{attempts}"
    )
