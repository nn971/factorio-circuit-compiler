...
@dataclass(frozen=True, slots=True)
class AbstractCompilationResult:
    """Result of the migrating abstract-physical pipeline."""
...
def compile_circuit(
    ...
    *,
    ...
    physical = (
        lower_with_alu_packing(optimized_semantic, state_timing=state_timing)
        if optimize
        else naive
    )
    return CompilationResult(
        ...
        blueprint_json=to_blueprint_json(physical, safe_wire_span=blueprint_safe_wire_span),
        blueprint_string=encode_blueprint_string(
            physical, safe_wire_span=blueprint_safe_wire_span
        ),
    )
...
def compile_abstract_circuit(
    ...
    *,
    ...
    """Compile through abstract physical synthesis.

    Scalar/stateless operations, whole-vector values, lane extraction, AccumulatorReg, and
    FreezeReg are supported. The existing ``compile_circuit`` backend remains the default
    while the new backend is validated and physical synthesis is refined.
    """
    ...
    abstract_physical = lower_abstract_physical(
        optimized_semantic, enable_packing=optimize, state_timing=state_timing
    )
    layout = synthesize_layout(
        abstract_physical, safe_wire_span=blueprint_safe_wire_span
    )
