"""Observed facade for the reach-safe joint annealer.

The implementation lives in :mod:`_incremental_joint_layout_core`. This facade deliberately keeps
all non-observability behavior delegated to that implementation while replacing only the annealing
hot loop with a decision-for-decision equivalent that can update a context-local statistics
collector. When no collector is active, callers see the same optimization decisions and artifact as
before.
"""
...
from factorio_circuit.synthesis import _incremental_joint_layout_core as _core
...
_ORIGINAL_ANNEAL_FEASIBLE = _core._anneal_feasible
...
_ACTIVE_STATS: ContextVar[_MutableOptimizationStats | None] = ContextVar(
    "factorio_joint_anneal_stats",
    default=None,
)


@contextmanager
def _collect_optimization_stats() -> Iterator[_MutableOptimizationStats]:
...
def _record_geometry_rejection(stats: _MutableOptimizationStats | None) -> None:
    if stats is not None:
        stats.geometry_rejections += 1


def _complete_epoch(
    stats: _MutableOptimizationStats | None,
    *,
...
def _anneal_feasible(
    ...
    topology: _core._FeasibleTopology,
    options: PlacementOptions,
    grid: base_placement._GridGeometry,
    diagnostics: list[str] | None = None,
) -> _core._FeasibleTopology:
    """Run the unchanged feasible annealer while optionally collecting observational counters."""

    stats = _ACTIVE_STATS.get()
    automatic_io = _core._automatic_io_entity_ids(state.circuit, options)
    ...
    center = _core._centroid([*state.positions.values(), *state.relay_positions.values()])
    unit_sites = set(grid.unit_slots)
    wide_sites = set(grid.slots)
    occupancy = _core._SpatialOccupancy.build(state)
    rng = Random(options.random_seed ^ 0x61A7E5ED)
    initial_body_envelope = _core._occupied_envelope(state)
    ...
    def target_envelope(scale: float) -> _core.Envelope:
    ...
    best_score = _core._exact_score(state, topology, center)
    if stats is not None:
        stats.best_objective_history.append(best_score)
    ...
    topology_rebuilds = {
        min(
            iterations,
            ceil(iterations * fraction / _core._EPOCH_PROPOSALS) * _core._EPOCH_PROPOSALS,
        )
        for fraction in _core._ANNEAL_REBUILD_FRACTIONS
    }

    for epoch_start in range(0, iterations, _core._EPOCH_PROPOSALS):
        epoch_end = min(iterations, epoch_start + _core._EPOCH_PROPOSALS)
        ...
        envelope_scale = 1.0 - (1.0 - _core._FINAL_ENVELOPE_SCALE) * shrink_progress
        current_target_envelope = target_envelope(envelope_scale)

        outliers = [
            item
            for item in movable_objects
            if _core._rectangle_overflow(
                state,
                item,
        ...
        for step in range(epoch_start, epoch_end):
            ...
            target = _core._proposed_position(
                state,
                object_id,
                grid,
                preferred,
            ...
            if owners:
                ...
                if state.object_half_extent(candidate) != state.object_half_extent(object_id):
                    _record_geometry_rejection(stats)
                    continue
                if not _core._position_is_legal(state, candidate, current, unit_sites, wide_sites):
                    _record_geometry_rejection(stats)
                    continue
                other = candidate
                if stats is not None:
            ...
            if not _core._position_is_legal(state, object_id, target, unit_sites, wide_sites):
                _record_geometry_rejection(stats)
                continue
            ...
            overflow_delta = sum(
                _core._rectangle_overflow(state, item, position, current_target_envelope)
                - _core._rectangle_overflow(
                    state,
                    item,
                    state.object_position(item),
                    current_target_envelope,
            ...
            delta = wire_delta + compact_delta + _core._ENVELOPE_OVERFLOW_WEIGHT * overflow_delta
        ...
        topology = _core._simplify_feasible_topology(state, topology)
        if stats is not None:
        ...
        if epoch_end in topology_rebuilds and epoch_end < iterations:
            ...
            if stats is not None and can_rebuild:
                stats.topology_rebuild_attempts += 1
            prior_topology = topology
            topology = _core._try_rebuild_annealed_topology(
                state,
                topology,
                grid,
                diagnostics=diagnostics,
            )
            if stats is not None and can_rebuild and topology is not prior_topology:
                stats.topology_rebuild_successes += 1
        occupancy = _core._SpatialOccupancy.build(state)
        score = _core._exact_score(state, topology, center)
    ...
    topology = _core._FeasibleTopology.build(state, best_routing)
    return _core._rebuild_automatic_interface_topology(state, topology, options, grid)


# Keep the unchanged implementation's public entry point on the observed hot loop as well. This is
# assignment rather than a second implementation of bootstrap/refinement behavior.
_core._anneal_feasible = _anneal_feasible
