...
search. The result is still an abstract coarse placement: C5 will be responsible for expanding a
candidate back to implementation coordinates, rebuilding relays transactionally, and exact-validating
...
def macro_placement_metrics(
    ...
    return MacroPlacementMetrics(
        width,
        height,
        area,
        geometry.implementation_area,
        occupancy,
        hpwl,
    )
...
def _legalize_targets(
    ...
    *,
    ...
    fixed = [
        index for index, macro in enumerate(geometry.level.macros) if macro.fixed
    ]
    for index in fixed:
...
def try_macro_zoom(
    ...
    *,
    ...
    footprint_area = sum(
        4.0 * half[0] * half[1] for half in geometry.half_extents
    )
...
def compact_macro_geometry(
    ...
    hyperedges: tuple[ImplementationHyperedge, ...],
    *,
    scales: tuple[float, ...] = (0.25, 0.35, 0.50, 0.65, 0.80, 0.90),
    max_legalization_radius: int | None = None,
) -> MacroZoomResult:
