...
combinator" assumption comes from legacy behavior/documentation and is not a valid basis for this
compiler's Factorio 2.x target.
...
- **No exact numeric capacity is currently recorded as a project invariant.** Do not infer, recall,
  or invent one. If an exact current-game upper bound matters, establish it from the current
  Factorio 2.x game/blueprint format or an explicit in-game probe, then record the result here.
...
When a prior conversation established only a qualitative fact (for example, "more than 20"), preserve
that exact strength. Do not reconstruct a more specific numeric claim from memory.
